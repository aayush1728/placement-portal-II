...
**Placement Cell** solves this by providing a centralised web platform where:
...
- **Daily deadline reminders** — runs every 24 hours and emails students about drives closing in the next 3 days
- **Monthly activity report** — runs on the first of every month and sends the admin an HTML report with placement stats
- **CSV export** — when a student triggers this from their dashboard, it runs as an async job in the background and emails them the file when done
...
You need Python 3.10+, a conda environment, and Redis running locally.
...
**Set up email (optional — only needed for the background job emails):**
...
```
To get an app password: Gmail → Security → 2-Step Verification → App Passwords → Generate one.

**Start everything — you need 4 terminals:**

Terminal 1 — the Flask API:
```bash
cd backend
python app.py
```
The API runs at `http://localhost:5000`. On first run, the admin account is created automatically.

Terminal 2 — Celery worker (handles background tasks):
cd "C:\Users\Aayush\Projects\MAD 2\ppa_v2_appdev2\placement_portal_II\frontend"
...
Open `frontend/index.html` using VS Code Live Serve.
</br>Or
```bash
cd frontend
python -m http.server 5500
